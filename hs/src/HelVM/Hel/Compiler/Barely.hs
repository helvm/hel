...
{- HLINT ignore -}
module HelVM.Hel.Compiler.Barely where
...
debruijn m n e = case e of
  { E x -> case x of
    ...
    -- More principled perhaps:
    -- ; StrCon s -> debruijn m n $ listify $ map (E . Const . ord) s
...
instantiate' t n tab = case t of
  ...
  ; TV s -> case lookup s tab of
    { Nothing -> let { va = TV (showInt n "") } in ((va, succ n), (s, va):tab)
    ; Just v -> ((v, n), tab)
    }
  ; TAp x y ->
